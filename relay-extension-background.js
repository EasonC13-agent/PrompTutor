...
// --- Auto-attach: automatically attach tabs without manual clicking ---

let autoAttachEnabled = true // flip to false via storage if needed

async function tryAutoAttach(tabId) {
...
// Auto-attach when a tab finishes loading
chrome.tabs.onUpdated.addListener((tabId, changeInfo) => {
  if (changeInfo.status === 'complete') {
    void tryAutoAttach(tabId)
  }
})

// Auto-attach all existing tabs on service worker startup
async function autoAttachAllTabs() {
  ...
  for (const tab of allTabs) {
    if (tab.id) void tryAutoAttach(tab.id)
  }
}

// Run on startup (service worker wake)
void autoAttachAllTabs()

// Load auto-attach preference
chrome.storage.local.get(['autoAttach'], (result) => {
  if (result.autoAttach === false) autoAttachEnabled = false
})
